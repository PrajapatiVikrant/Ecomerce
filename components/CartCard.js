...
async function deleteItem(){
    const deleteRes = await axios.delete(`https://expert-carnival-ww9776xrgqpfg6wj-3000.app.github.dev/api/cart/${productData._id}`,{
        headers:{
          Authorization: `Bearer ${localStorage.getItem('token')}`, 
        }
      } )
      if(deleteRes.data == 'deleted'){
      const data = await axios.get('https://expert-carnival-ww9776xrgqpfg6wj-3000.app.github.dev/api/cart',{
        headers:{
          Authorization: `Bearer ${localStorage.getItem('token')}`, 
        }
      } )
...
async function editItem(){
    const editRes = await axios.put(`https://expert-carnival-ww9776xrgqpfg6wj-3000.app.github.dev/api/cart/${productData._id}`,{qty:editvalue},{
        headers:{
          Authorization: `Bearer ${localStorage.getItem('token')}`, 
        }
      } )
